...
export class Game {
  ...
  draw() {
    ...
    const x = this.canvas.width * 1 / 9
    const y = this.canvas.height * 1 / 9
    this.ctx.beginPath()
    this.ctx.rect(sx, sy, x, y)
    this.ctx.fill()
    this.ctx.restore()
  }
  ...
  update() {
    ...
    if (this.controller.isMouseInsideRect(sx, sy, x, y) && this.controller.keys.includes('mousedown') && this.clickAcceptable) {
      this.count++
